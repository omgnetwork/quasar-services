...
const faucet = {
  ...
  async initEthBalance(minAmount, topupMultipler) {
  ...
  async initERC20Balance(minAmount, topupMultipler) {
    if (numberToBN(minAmount).eqn(0)) {
      return;
    }
  ...
  async topUpERC20(amount, address) {
  ...
  async fundChildchain(address, amount, currency) {
  ...
  async fundRootchainEth(address, amount) {
    if (amount <= 0) {
      return;
    }
    const txDetails = {
  ...
  async fundRootchainERC20(address, amount) {
    if (amount <= 0) {
      return;
    }
  ...
  async returnChildchainFunds(from, to = this.faucetAccount) {
  ...
  async returnRootchainFunds(from, to = this.faucetAccount) {
    try {
